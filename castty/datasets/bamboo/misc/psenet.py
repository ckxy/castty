...
class PSEEncode(BaseInternode):
    ...
    def forward(self, data_dict, **kwargs):
        ...
        data_dict['pse_kernel'] = torch.from_numpy(kernels)

        pse_mask = generate_effective_mask((w, h), data_dict['poly'], ignore_flags)
        data_dict['pse_mask'] = torch.from_numpy(pse_mask)

        # print(data_dict['pse_kernel'].shape, data_dict['pse_mask'].shape)
        # exit()
...
class PSECrop(CropInternode):
    ...
    def calc_cropping(self, data_dict):
        assert 'pse_kernel' in data_dict.keys()

        w, h = get_image_size(data_dict['image'])

        img_gt = data_dict['pse_kernel']
