...
class CalcNanoGrids(BaseInternode):
    def __init__(self, scale, strides, num_classes, top_k=9, analysis=False):
        self.scale = scale
    ...
    def __call__(self, data_dict):
        ...
        bboxes, labels, weights = torch.split(torch.from_numpy(data_dict['bbox']).type(torch.float), (4, 1, 1), dim=1)
        ...
        # print(pos_inds.numel(), neg_inds.numel())
    ...
    def rper(self):
        return 'CalcNanoGrids(not available)'
